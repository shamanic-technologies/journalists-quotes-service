...
| `POST` | `/orgs/opportunities/ranked` | apiKey + orgId + x-brand-id | **Pure-read** paginated list of RAG-scored opportunities for the brand-set. Body: `{ campaignId?, limit?, offset? }`. Reads from `quote_priorities` (populated by `/next`); never scores synchronously. Annotated with latest `pitchStatus`. Used by HITL dashboard + public report queue. |
| `GET` | `/orgs/opportunities/stats` | apiKey + orgId + x-brand-id | Brand-set scoped catalog metrics. Query: `?campaign_id=` (optional). Returns `{ silverPoolSize, scoredCount, eligibleCount, pitchedBlocking, expiredCount, bestEligibleScore, brandIds }`. Pure read. |
